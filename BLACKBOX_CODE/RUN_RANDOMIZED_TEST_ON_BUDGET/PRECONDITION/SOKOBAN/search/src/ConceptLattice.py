...
SAMPLES_DIR = "../DATA/SAMPLES/SOKOBAN/"
...
#                self.concept_list.append(i)
    ...
    def search_from_files(self,current_sampling_budget_list=None):
        ...
        for cid in range(10):
            ...
            #    current_sampling_budget = SAMPLES_BUDGET 
            ...
            if len(LOG_LIST) > 0:
                print ("\n".join("[LOGS] "+i for i in LOG_LIST))
                LOG_LIST = []
            for budget in current_sampling_budget_list:
                sample_cnt = 0
                for sfile in all_files:
                    current_sampling_budget=budget
                    sample_cnt +=1
                    ...
                    if len(CURR_POSS_CONCEPTS) == 0:
                        print ("The file where it failed", sfile)
                        self.class_obj.save_test_imge(act_seq)
                    ...
                    if sample_cnt >= current_sampling_budget:
                        ...
                        print ("############### FINAL OUTPUT######################")
                        for rem_conc in CURR_POSS_CONCEPTS:
                            print ("remaining_concept", rem_conc)
                            print ("Probability",CURR_POSS_CONCEPTS_MAP[rem_conc])
                        
                        #if os.path.exists('/tmp/switch_on_prob_'+str(cid#)):
                        #    with open('/tmp/switch_on_prob_'+str(cid)) as s_fd:
                        #        current_list = [i.strip() for i in s_fd.readlines()]
                        #else:
                        #    current_list = []
                        LOG_LIST.append(str(current_sampling_budget)+":"+str(len(CURR_POSS_CONCEPTS))+","+ str(CURR_POSS_CONCEPTS_MAP['concept_switch_on'])+","+str(time.time() - start_time))
                        #with open('/tmp/switch_on_prob_'+str(cid), 'w') as s_fd:
                        #    s_fd.write("\n".join(current_list))
                        #return None
                        break
        ...
        print ("############### FINAL OUTPUT######################")
        #for conc_id in range(len(CURR_POSS_CONCEPTS)):
        #    print ("remaining_concept", CURR_POSS_CONCEPTS[conc_id])
        #    print ("Probability",CURR_POSS_CONCEPTS_MAP[CURR_POSS_CONCEPTS[conc_id]])
        return True 
