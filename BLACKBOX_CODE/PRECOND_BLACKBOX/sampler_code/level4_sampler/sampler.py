...
class Sampler:
    ...
    def set_starting_state(self):
        ...
        if self.sampling_type == "vanilla_random":
            ...
            file_prefix = "../config/MontezumaL4Config/"
            plan_file = file_prefix + "action_sequence.b"
            level_init_file = file_prefix + "level3init.b"
            with open(level_init_file,'rb') as ld:
                init_act_seq = pickle.load(ld)

            with open(plan_file, 'rb') as fp:
                action_sequence = pickle.load(fp)

            random_state_idx = random.choice(range(len(action_sequence)))
            self.env.reset()
            for action in init_act_seq + action_sequence[:random_state_idx+1]:
                _ = self.env.step(action)
                assert _[3]['ale.lives'] == 6
            print("Starting from state-%d" % random_state_idx)

            return init_act_seq + action_sequence[:random_state_idx+1]
        else:
            self.env.reset()
            return []
    ...
    def run_episodes(self):
        ...
        for i_episode in range(self.num_episodes):
            ...
            while True:
                ...
                if next_state_info[3]['ale.lives'] < 6 or act_cnt >= 200 or ram[3] !=4:
    ...
    def save_state(self, plan=None):
        ...
        print ("Samples count", sample_idx)
...
if __name__ == "__main__":
    conditions = ["on_rope"]
        #, "on_ground_and_alive", "on_ladder1", "on_ladder2", "on_ladder3",
